...
_float_speedLimitMin = 0;
_float_speedLimitMax = 200;
_float_speedApprox = 2;

_float_fwdRatioMin = 0.1;
_float_fwdRatioMax = 0.8;

_float_turnApprox = 60;
...
_jmpm = createMarkerLocal ["jtarg", [0,0,0]];
_jmpm setMarkerShapeLocal "ICON";
_jmpm setMarkerTypeLocal "Select";
_jmpm setMarkerColorLocal "ColorGreen";
_jmpm setMarkerSizeLocal [0.5, 0.5];

while{ _obj_jump getVariable "bool_freeFall" }do
{		
	_jmpm setMarkerPosLocal position _obj_jump;
	...
	[
		_float_speedH*_float_vX+(random 10),
		_float_speedH*_float_vY+(random 3),
		(-1*_float_speedV)-20
	];
	...
	if(_float_angle < 10) then {_obj_jump switchMove "para_pilot"}else{_obj_jump switchMove "kia_para_pilot"}; 
	
	ctrlSetText[ 857,format[ "%1|%2",round(_float_speedH ),round( getPos _obj_jump select 2)] ];
	//ctrlSetText[ 857,format[ "%1",round( getPos _obj_jump select 2 ) ] ];
	_main = finddisplay 856;
	_ctrl = _main displayctrl 858;
	ctrlMapAnimClear _ctrl;
	_speed = (_float_speedH)+1;
	_ctrl ctrlMapAnimAdd [0, _speed/100, (position _obj_jump)];
