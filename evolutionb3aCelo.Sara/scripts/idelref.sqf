...
if(_type == 2 and (leader _group) distance _pos < 300) exitWith {_handle = [_group] execVM "scripts\delete.sqf"}; 


//hint format ["%1",list _alist];
...
for [{_loop=0}, {_loop<1}, {_loop=_loop}] do
{
	...
	//hint format["enemy= %1",((leader _group) findNearestEnemy position (leader _group))];
	sleep 1.0;
};
